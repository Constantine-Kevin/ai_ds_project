...
def naivebayes(word_list,pos_dict,neg_dict):
    ...
    for i in word_list:
        #P(i|positive)
        p_positive *= (pos_dict.get(i,0)+to_add)/(sum(pos_dict.values()) + len(pos_dict)*(to_add)+ to_add)
        #P(i|negative)
        p_negative *= (neg_dict.get(i,0)+to_add)/(sum(neg_dict.values()) + len(neg_dict)*(to_add) + to_add)
